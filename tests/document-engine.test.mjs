...
e.firmarComoElaborador(plan.id,'demo.p12',`Página ${planSignaturePage}`);
...
e.generarArtefacto(plan.id,data);
e.firmarComoElaborador(plan.id,'demo.p12',`Página ${doc(plan.id).currentArtifact.signatureSlots[0].pageNumber}`);
...
e.firmarComoElaborador(report.id,'demo.p12',`Página ${doc(report.id).currentArtifact.signatureSlots[0].pageNumber}`);
